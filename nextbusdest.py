...
import requests
from busData import stops, routes
...
def destOnRoute(route, dest):
    for s in routes[str(route)]:
        print('{0} {1}'.format(dest, getTagStop(s)))
...
def getDestETA(route, start, dest):
    jsonRt = requests.get('http://nextbus.nodejitsu.com/route/{}'.format(route))
...
def nextBus(start):
    ...
    return render_template('nextdest.html', bus=nextbus, smins=next, dest=dest, dmins=eta)
